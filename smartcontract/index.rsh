    ...
    const [keepGoing, howMany, total] = parallelReduce([true, 0, 0])
        .define(() => {
            ...
            const register = (parent_address, user_address) => {
                ...
                check(!(fromSome(users[user_address], D) == user_address), "Already a member sorry");
                check(fromSome(numChildren[parent_address], 0) < 2, "No empty slots for that user");

                return () => {
                    numChildren[parent_address] = fromSome(numChildren[parent_address], 0) + 1;
                    parentMap[user_address] = parent_address;
                    users[user_address] = user_address;
                    totalKids[parent_address] = fromSome(totalKids[parent_address], 0) + 1;
                    allocatedAmount[user_address] = 0;
                    allocatedAmount[parent_address] = fromSome(allocatedAmount[parent_address], 0) + price;
            ...
            const withdraw_accumulated_funds = (user_address, confirm) => {
                ...
                check(!(fromSome(allocatedAmount[user_address], 0) == 0),
